...
export const authSchema = Joi.object().keys({
    ...
    password:Joi.string().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.{8,})/).label('password is required, must be at least 8 characters and must contain at least a number, one lowercase and one uppercase alphabet'),
    email:Joi.string().trim().lowercase().email().required().label('email is required, and should follow this format: myemail@domain.com'),
